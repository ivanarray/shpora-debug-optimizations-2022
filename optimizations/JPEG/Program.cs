...
namespace JPEG
{
    class Program
    {
        ...
        private static readonly IReadOnlyList<Func<Pixel, float>> ChannelSelectors
...
//				var fileName = "Big_Black_River_Railroad_Bridge.bmp";
                ...
                var imageMatrix = GetImageMatrix(fileName);

                sw.Stop();
                Console.WriteLine($"LoadingImage: {sw.Elapsed}");
        ...
        private static Matrix GetImageMatrix(string fileName)
        {
        ...
        {
            var allQuantizedBytes = new List<byte>(2000_000);
        ...
        private static void SetPixels(Matrix matrix, float[,] a, float[,] b, float[,] c, PixelFormat format,
        ...
        private static IReadOnlyList<byte> ZigZagScan(byte[,] channelFreqs)
        {
            return new[]
            {
